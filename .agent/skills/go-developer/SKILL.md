...
- **Specs & Requirements**: Use `createConfluencePage` to write Technical Specs.
...
**Do not guess the workflow. Read the file.**
...
11. **Never modify or commit `.gitignore`** - Do not manage this file unless explicitly requested by the user.

---

## Verification & Safety Rules (STRICT ENFORCEMENT)

1. **Local Code Check Before Commit & Push:**
   - Before committing OR pushing code to *any* branch, YOU MUST verify all code.
   - Run `go fmt`, `go vet`, and `go test ./...` to ensure no regressions.
   - YOU MUST run `golangci-lint run` before every commit. If there are ANY lint errors, they MUST be fixed before the code is committed.
